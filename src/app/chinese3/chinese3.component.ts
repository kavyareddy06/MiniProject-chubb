...
@Component({
  selector: 'app-chinese3',
  standalone: true,
  imports: [RouterLink, NavbarComponent, CommonModule],
  templateUrl: './chinese3.component.html',
  styleUrl: './chinese3.component.css'
})
export class Chinese3Component {
  chineseItems: Chinese3Item[] = [
    {
  ...
  increaseQuantity(item: Chinese3Item) {
    item.quantity += 1;
    this.cartService.updateItemQuantity(item.name, item.quantity);
  }

  decreaseQuantity(item: Chinese3Item) {
    if (item.quantity > 0) {
      item.quantity -= 1;
      this.cartService.updateItemQuantity(item.name, item.quantity);
    }
  ...
  toggleFavorite(item: Chinese3Item) {
  ...
  isFavorite(item: Chinese3Item): boolean {
